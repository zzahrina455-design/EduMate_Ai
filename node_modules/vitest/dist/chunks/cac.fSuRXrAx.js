...
async function start(cliFilters, options) {
	try {
		const { startVitest } = await import('./cli-api.DcLieX4F.js').then(function (n) { return n.a; });
...
async function doctorCommand(cliFilters, options) {
	try {
		const { doctor } = await import('./doctor.DR3u0Z_G.js');
...
async function init(project) {
	...
	const { create } = await import('./creator.BEj8pIIH.js');
	await create();
}
async function collect(cliFilters, options) {
	try {
		const { prepareVitest, processCollected, outputFileList } = await import('./cli-api.DcLieX4F.js').then(function (n) { return n.a; });
