...
import { a as relative } from './chunk-pathe.M-eThtNZ.js';
...
function hoistMocks(code, id, parse, options = {}) {
				...
				if (hoistableMockMethodNames.includes(methodName)) {
					...
					if (options.onStaticMock) {
						...
						if (specifier != null) {
							...
							const factory = node.arguments[1]?.type === "ArrowFunctionExpression" || node.arguments[1]?.type === "FunctionExpression" ? node.arguments[1] : undefined;
							options.onStaticMock({
								method: methodName,
								specifier,
								hasFactory: factory != null,
								factoryLoadsOriginal: factory != null && (factory.params.length > 0 || code.slice(factory.start, factory.end).includes("importActual"))
							});
						}
					}
	...
	if (!hasImportMetaVitest) {
		...
		if (hoistedNodes.size) {
			...
			const map = options.getMap && new TraceMap(options.getMap());
			...
			const message = [
				...
				...Array.from(hoistedNodes, (invalidNode) => {
					...
					const location = originalLocation?.column != null && originalLocation?.line != null ? ` at ${relative(options.root || process.cwd(), id)}:${originalLocation.line}:${originalLocation.column + 1}` : "";
...
function getStaticSpecifier(node) {
	if (node?.type === "AwaitExpression") {
		node = node.argument;
	}
